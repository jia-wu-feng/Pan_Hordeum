...
list_s9<-c("Hordeum_bogdanii..Hordeum_vulgare","Hordeum_brevisubulatum..Hordeum_vulgare",
           ...
           "Hordeum_vulgare..Hordeum_euclaston","Hordeum_parodii_Sub3..Hordeum_vulgare",
           "Hordeum_parodii_Sub2..Hordeum_vulgare","Hordeum_parodii_Sub1..Hordeum_vulgare",
           "Hordeum_gussoneanum_Sub2..Hordeum_vulgare","Hordeum_gussoneanum_Sub1..Hordeum_vulgare")
...
fread("pericen_all.txt",header = F)-> bjbed

pdf("boundary.pdf",6,6)
for(splist in list_s9){
  ref_species=unlist(strsplit(splist,"[..]"))[1]
  qry_species=unlist(strsplit(splist,"[..]"))[3]
  print(paste(ref_species,qry_species))
  for(chr in c("chr1H","chr2H","chr3H","chr4H","chr5H","chr6H","chr7H")){
    x_bar <- ggplot(finalcov[V1==chr&ref==ref_species], aes(x = V2/1e6, y = ncov,fill = color)) +
      geom_bar(stat = "identity") + xlab("")+ylab("NCoverage")+
      geom_vline(data = bjbed[V1==chr&V4==ref_species], aes(xintercept = V2) ,linetype=4,linewidth=1,color="blue")+
      geom_vline(data = bjbed[V1==chr&V4==ref_species], aes(xintercept = V3),linetype=4,linewidth=1,color="blue")+
      theme_minimal() + theme(legend.position = "null")
    main_plot <- ggplot(finalpafl[mapq>=20&reference==chr&refsp==ref_species], aes(x = reference_start/1e6, y = query_start/1e6, xend = reference_end/1e6, yend = query_end/1e6)) +
      xlab("")+ylab(paste(ref_species,chr,"Alignment"))+
      geom_vline(data = bjbed[V1==chr&V4==ref_species], aes(xintercept = V2) ,linetype=4,linewidth=1,color="blue")+
      geom_vline(data = bjbed[V1==chr&V4==ref_species], aes(xintercept = V3),linetype=4,linewidth=1,color="blue")+
      geom_segment(arrow = arrow(length = unit(0.01, "inches")))+
      theme_minimal()+theme(legend.position = "null")
